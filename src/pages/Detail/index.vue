        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="chooseArea">
              ...
              <dl v-for="(disposition,index) in this.$store.getters.spuSaleAttrList" :key="index">
                <dt class="title">{{ disposition.saleAttrName }}</dt>
                <dd
                    changepirce="90"
                    v-for="option in disposition.spuSaleAttrValueList"
                    :key="option.id"
                    :class="{active:option.isChecked==1}"
                    @click="changeOption(option,disposition.spuSaleAttrValueList)"
                >{{ option.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            //购买数量
            <div class="cartWrap">
              ...
              <div class="add">
                <a @click="AddOrUpdateShopCard">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    skuid() {
      return this.$store.getters.skuInfo.skuid
    }
  },
  methods: {
    changeOption(option, arr) {
      //遍历，并将所有的isChecked设为0
      arr.forEach(item => {
        item.isChecked = 0
      })
      //将点击的isChecked值设为1
    ...
    async AddOrUpdateShopCard() {
      //让用户获得添加购物车是否成功的结果
      //将当前商品数据存入sessionStorage，方便以后使用

      try {
        sessionStorage.setItem('SKUINFO', JSON.stringify(this.skuInfo))
        await this.$store.dispatch('reqAddOrUpdateShopCard', {skuid: this.$store.getters.skuInfo.id, skunum: this.skuNum})
        //携带数据
...
.detail .con .mainCon .InfoWrap .choose .chooseArea dl dd:hover {
  cursor: pointer;
}
...
.detail .product-detail .aside .tabContent .tab-pane:nth-child(1) .goodsList > li .list-wrap .p-img {
  text-align: center;
}

.detail .product-detail .aside .tabContent .tab-pane:nth-child(1) .goodsList > li .list-wrap .p-img img {
  width: 152px;
}

.detail .product-detail .aside .tabContent .tab-pane:nth-child(1) .goodsList > li .list-wrap .price {
  font-size: 16px;
  color: #c81623;
}

.detail .product-detail .aside .tabContent .tab-pane:nth-child(1) .goodsList > li .list-wrap .operate {
  text-align: center;
  margin: 5px 0;
}

.detail .product-detail .aside .tabContent .tab-pane:nth-child(1) .goodsList > li .list-wrap .operate a {
