<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(trade,index) in tradeInfo.detailArrayList" :key="index">
          <li>
...
export default {
  ...
  mounted() {
    this.$store.dispatch('reqGetUserAddress')
    this.$store.dispatch('reqGetUserTrade')
  },
  computed: {
    ...mapGetters(['addressList', 'tradeInfo']),
    defaultAddress() {
  ...
  methods: {
    ...
    async submitTrade(){
      ...
      const result = await this.$API.reqSubmitTrade(tradeNo, data)
      if(result.code===200){
  ...
  .content {
    ...
    .address {
      ...
      .username::after {
        ...
        right: 0;
        bottom: 0;
        background: url(@/pages/Trade/images/choosed.png) no-repeat;
      }
